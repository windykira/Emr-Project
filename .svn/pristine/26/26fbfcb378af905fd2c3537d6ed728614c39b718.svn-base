...
public class DcController extends BaseController {
    ...
    public String getTemplate(HttpServletRequest request, @PathVariable String fileId) throws IOException {
        ...
        String xml = MyFileUtil.readFile(SystemConfigParseUtil.getProperty("TEMP_FILE_PATH") + path);
        return xml;
    }
    ...
    public String getXML(HttpServletRequest request, @PathVariable String fileId) throws IOException {
        String xml = MyFileUtil.readFile("C:\\dc\\tgjc\\既往史.xml");
        return xml;
    }
    @RequestMapping(value="/saveSub")
    @ResponseBody
    public String saveSub(@RequestParam Map<String, Object> params) throws IOException {
        String xml = params.get("xml").toString();
        MyFileUtil.writeFile(SystemConfigParseUtil.getProperty("SUB_FILE_PATH")+params.get("pid")+"\\",params.get("type")+".xml",xml);
        return "";
    }
    @RequestMapping(value="/getSubXML")
    @ResponseBody
    public String getSubXML(@RequestParam Map<String, Object> params) throws IOException {
        String id = params.get("id").toString();
        String type = params.get("type").toString();
        String xml = MyFileUtil.readFile(SystemConfigParseUtil.getProperty("SUB_FILE_PATH")+id+"\\"+type+".xml");
        return xml;
    }
    @RequestMapping(value="/getTemplateByCatalog/{id}")
    @ResponseBody
    public Map getTemplateByCatalog( @PathVariable String id) throws IOException {
        Map m = service.getTemplateByCatalogId(id);
        String path = m.get("CONTENT").toString();
        String jsFileName = m.get("JS_FILE_NAME").toString();
        Map resultMap = new HashMap();
        String xml = MyFileUtil.readFile(SystemConfigParseUtil.getProperty("TEMP_FILE_PATH") + path);
        resultMap.put("xml",xml);
        resultMap.put("jsFileName",jsFileName);
        return resultMap;
    }
}
