...
function isModified(){
    ...
    if(!inputIsModified("诊断依据")) return false;
    if(!inputIsModified("鉴别诊断")) return false;
    if(!inputIsModified("诊疗计划")) return false;
    return true;
}
//----以下为实现----
...
function click(eventSender,eventArgs){
    if(eventSender.CurrentInputField) {
        ...
        if(eventSender.CurrentInputField.name=="入院检查"){
            openReportNew('1');
        }
        if(eventSender.CurrentInputField.name=="入院检验"){
            openReportNew('2');
        }
    }
}
...
//数据共享
    ...
    loadInputByType(id,"鉴别诊断","鉴别诊断");
    loadInputByType(id,"体格检查","入院体格检查");
    loadInputByType(id,"诊断","入院诊断");
    loadInputByType(id,"诊断依据","诊断依据");
    loadInputByType(id,"诊疗计划","诊疗计划");
